...
#endif
DWORD RFWASP_SetSWTraceEvent_log0(BYTE byChNo, const RFW_SWTRC_DATA* pSwTrcData)
{
	// RHはentryでは何もしない。exitでまとめてログ出力する
	return RFWERR_OK;
}
#if defined(_MSC_VER)
#pragma warning(pop)
...
DWORD RFWASP_SetSWTraceEvent_log1(BYTE byChNo, const RFW_SWTRC_DATA* pSwTrcData, DWORD dwRet)
{
	if (!DebugMsgFlgChk(ASP_FUNC_GR)) {
		// ログ出力しない
		return RFWERR_OK;
	}
	IniMsgBuf_RFW(ASPCMD_SETSWTRCEV, dwRet);

	//  全パラメータの出力に対応
	SetMsgBufData1(683, CHARTYPE, byChNo);
	if (pSwTrcData) {
		SetMsgBufData1(684, CHARTYPE, pSwTrcData->byTraceType);
		SetMsgBufData1(685, CHARTYPE, pSwTrcData->byRegister);
		SetMsgBufData1(686, LONGTYPE, pSwTrcData->dwData);
		SetMsgBufData1(687, LONGTYPE, pSwTrcData->dwMask);
	}
...
#pragma warning(disable:4100)
#endif
DWORD RFWASP_GetSWTraceEvent_log0(BYTE byChNo)
{
	// RHはentryでは何もしない。exitでまとめてログ出力する
	return RFWERR_OK;
}
...
DWORD RFWASP_GetSWTraceEvent_log1(BYTE byChNo, RFW_SWTRC_DATA* pSwTrcData, DWORD dwRet)
{
	if (!DebugMsgFlgChk(ASP_FUNC_GR)) {
		// ログ出力しない
		return RFWERR_OK;
	}
	IniMsgBuf_RFW(ASPCMD_GETSWTRCEV, dwRet);

	//  全パラメータの出力に対応
	SetMsgBufData1(688, CHARTYPE, byChNo);
	SetMsgBufData1(689, CHARTYPE, pSwTrcData->byTraceType);
	SetMsgBufData1(690, CHARTYPE, pSwTrcData->byRegister);
	SetMsgBufData1(691, LONGTYPE, pSwTrcData->dwData);
	SetMsgBufData1(692, LONGTYPE, pSwTrcData->dwMask);
...
#pragma warning(disable:4100)
#endif
DWORD RFWASP_ClrSWTraceEvent_log0(BYTE byChNo)
{
	// RHはentryでは何もしない。exitでまとめてログ出力する
	return RFWERR_OK;
}
...
DWORD RFWASP_ClrSWTraceEvent_log1(BYTE byChNo, DWORD dwRet)
{
	if (!DebugMsgFlgChk(ASP_FUNC_GR)) {
		// ログ出力しない
		return RFWERR_OK;
	}
	IniMsgBuf_RFW(ASPCMD_CLRSWTRCEV, dwRet);

	//  全パラメータの出力に対応
	SetMsgBufData1(693, CHARTYPE, byChNo);
